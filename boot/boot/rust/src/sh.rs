...
use crate::util::{str::Str, args::{/*SEPARATOR, */pass as pass_args}};
...
use crate::vga;
use vga::{SCREEN_POS_VIRT, SCREEN_POS, rewrite_vga};
...
pub fn open() {
	unsafe {
		...
		print_bytes(b"\x0fshell started, type `help` for any information\x10\n");
		prompt();

		loop {

			match read(0, &mut s, &mut buffer) {

				Output::Ok => {

					if at {
						let char = pass_map(Key {char: key, press: transmute(buffer)});
...
components:
	numbers
command list:
	echo, println, print, help, fail, panic, poweroff, mem, port, term
");
...
number formats:
	...
	to write octal number you write: `0o173`, `o270000r0`",
		...
		} else if command == b"fail" || command == b"panic" {
