...
options:
	...
	--new-lines	shows you new number of lines putted after last checking (TODO)
"
b=0
run=0
			...
			if [ "`"$rh/rustc" --version | grep nightly`" != "" ]; then
				...
				if [ $release = 0 ]; then
					...
					if ~/.cargo/bin/cargo bootimage; then
						...
						cd ../../..
						cp boot/boot/rust/target/target/debug/bootimage-nierdzewny.bin out/output.bin
					...
					if "$rh"/cargo bootimage --release; then
						...
						read outputfile
						cp boot/boot/rust/target/target/release/bootimage-nierdzewny.bin "$outputfile"
